...
<html lang="en">
	...
	<body>
		...
		<script>
			fetch('../Doctor/doctorDashExec.php') 
				...
				.then(data => {
					...
					const upcomingAppointmentsTableBody = document.querySelector('#upcomingAppointments tbody');
					upcomingAppointmentsTableBody.innerHTML = ''; 

					if (data.upcomingAppointments.length === 0) {
						const row = document.createElement('tr');
						row.innerHTML = `
							<td colspan="5" class="no-appointments">No upcoming appointments</td>
						`;
